...
class UartBaseViewController: PeripheralModeViewController, UITextFieldDelegate  {
    // Config
    ...
    @IBOutlet weak var saveURLOutlet: UITextField!
    @IBOutlet weak var intervalLabel: UILabel!
    // UI
    @IBAction func saveURLAction(_ sender: Any) {
        UserDefaults.standard.set(saveURLOutlet.text, forKey: "url")
        self.saveURLOutlet.resignFirstResponder()
        
    }
    @IBAction func intervalAction(_ sender: UISlider) {
        intervalLabel.text = String(format:"%.1f",intervalOutlet.value)
    }
    @IBOutlet weak var intervalOutlet: UISlider!
    
    @IBAction func loopOnAction(_ sender: Any) {
        
        if loopOnOutlet.isOn {
            print("switch on")
            self.backgroundTask.startBackgroundTask()
            loopNS()
            loopTimer = Timer.scheduledTimer(timeInterval: TimeInterval(Int(intervalOutlet.value * 60)), target: self, selector: #selector(loopNS), userInfo: nil, repeats: true)
        }
        else
        {
            print("switch off")
    ...
    @IBOutlet weak var loopOnOutlet: UISwitch!
    ...
    override func viewDidLoad() {
        ...
        saveURLOutlet.returnKeyType = .done
        intervalOutlet.value = 1.0
        saveURLOutlet.text = UserDefaults.standard.string(forKey: "url")
        // Init Data
        keyboardPositionNotifier.delegate = self
        timestampDateFormatter.setLocalizedDateFormatFromTemplate("HH:mm:ss")
        
        DispatchQueue.main.async {
            self.intervalLabel.text = String(format:"%.1f", self.intervalOutlet.value)
        }
    ...
    func loopNS () {
        print ("loop ")
        let urlString = saveURLOutlet.text! + "/api/v1/entries.json"
        guard let url = URL(string: urlString) else {
            ...
            loopOnOutlet.setOn(false,animated:false)
        ...
        session.dataTask(with: request as URLRequest) { (data, response, error) in
            if error != nil {
                ...
                DispatchQueue.main.async {
                    ...
                    self.loopOnOutlet.setOn(false,animated:false)
                    self.present(alert, animated: true)}
                return
            }
            ...
            do {
                ...
                if entries.count > 0 {
                    ...
                    let bpayload = String(entries[0].sgv)
                    let dpayload = entries[0].direction
                    let tpayload = strDate
                    //self.send(message: bpayload)
                    //self.send(message: dpayload)
                    //self.send(message: tpayload)
                    self.send(message: bpayload + "," + dpayload + "," + tpayload)
                    DispatchQueue.main.async{
                        print(entries[0].sgv)
...
//        }
    ...
    override func viewWillAppear(_ animated: Bool) {
        super.viewWillAppear(animated)
         ...
         intervalLabel.text = String(format:"%.1",0.5)
        // Enable Uart
        setupUart()
